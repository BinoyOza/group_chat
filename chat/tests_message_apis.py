...
class TestUserAPIView(APITestCase):
    ...
    def test_should_send_message(self):
        ...
        sample_request = {
            "message": "This is an another sample to a group.",
            "sender": 2
        }
        response = self.client.post(reverse("send-message"), sample_request, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(response.data.get("success"))

    def test_should_view_messages(self):
        self.non_admin_authentication()
        self.create_test_objects()
        sample_request = {
        ...
        self.client.post(reverse("send-message"), sample_request, format="json")
        response = self.client.get("/chat/view-messages?group_id=1")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get("group_details")[0].get("group_id"), 1)
        self.assertEqual(len(response.data.get("group_details")[0].get("message_details")), 3)
